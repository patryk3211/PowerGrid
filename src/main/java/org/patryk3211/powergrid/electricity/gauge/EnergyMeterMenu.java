...
public class EnergyMeterMenu extends MenuBase<EnergyMeterBlockEntity> {
    public EnergyMeterMenu(MenuType<?> type, int id, Inventory inv, RegistryFriendlyByteBuf extraData) {
    ...
    @Override
    protected EnergyMeterBlockEntity createOnClient(RegistryFriendlyByteBuf extraData) {
        var world = Minecraft.getInstance().level;
        var be = world.getBlockEntity(extraData.readBlockPos());
        if(be instanceof EnergyMeterBlockEntity meter) {
            meter.readClient(extraData.readNbt(), extraData.registryAccess());
            return meter;
        }
        return null;
    }
