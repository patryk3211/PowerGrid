...
public class PlotterRenderer extends KineticBlockEntityRenderer<PlotterBlockEntity> {
    ...
    private static final float VOLTAGE_SPAN = 12.0f / 16;
    private static final Vec3 GRAPH_ORIGIN = new Vec3(8.0 / 16.0, 16.0 / 16.0, 13.0 / 16.0);
    ...
    protected void renderSafe(PlotterBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
        ...
        var pointer = CachedBuffers.partialFacing(ModdedPartialModels.PLOTTER_POINTER, state, facing.getOpposite());
        var paper = CachedBuffers.partialFacing(ModdedPartialModels.PLOTTER_PAPER, state, facing.getOpposite());

        pointer
                .light(light)
                .translate(partialTicks * 4 / 16f, 0, 0)
                .renderInto(ms, buffer.getBuffer(RenderType.solid()));
        paper
                .light(light)
                .shiftUVScrolling(ModdedPartialModels.PAPER_SHIFT, 0, 0)
                .renderInto(ms, buffer.getBuffer(RenderType.solid()));
